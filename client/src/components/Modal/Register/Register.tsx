...
export function Register(): JSX.Element {
  ...
  const [userData, setUserData] = useState<RegData>({ username: '', password: '', telephone: '', lastName: '', firstName: '', email: '' });
  ...
  const login = (data: RegData): ThunkAction<Promise<ResponseData>, IGeneralState, unknown, UserActionTypes> => async (dispatch: Dispatch<UserActionTypes>): Promise<ResponseData> => {
  ...
  if (responseData.token) {
    dispatch({ type: Types.REGISTER_SUCCESS, payload: userReady });
  } else {
                ...
                <div className="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                  <form onSubmit={onSubmit} className="w-full max-w-lg">
                    ...
                    <div className="flex flex-wrap -mx-3 mb-6">
                      ...
                      {error && (
                        <span className="absolute top-0 right-0 mt-2 mr-3 text-sm text-red-600">
                          {error}
                        </span>
                      )}
                    </div>
